...
class TransactionTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final signedAmount = transaction!.type == TransactionType.expense
        ? -transaction!.amount
        : transaction!.amount;
    final amountText = FormatUtils.formatCurrency(signedAmount, currencyCode: transaction!.currency);
    ...
    // Check if we have notes or tags to display
    final hasNote = transaction!.note?.isNotEmpty == true;
    final hasTags = transaction!.safeTags.isNotEmpty;
      ...
      child: ListTile(
        ...
        subtitle: Column(
          ...
          children: [
            ...
            if (hasTags) ...[
              Wrap(
                spacing: L.responsiveSize(phone: 4, tablet: 6, desktop: 8),
                runSpacing: L.responsiveSize(phone: 2, tablet: 3, desktop: 4),
                children: transaction!.safeTags.take(3).map((tag) => Container(
                  padding: EdgeInsets.symmetric(
                    horizontal: L.responsiveSize(phone: 6, tablet: 8, desktop: 10),
                    vertical: L.responsiveSize(phone: 2, tablet: 3, desktop: 4),
                  ),
                  decoration: BoxDecoration(
                    color: baseColor.withValues(alpha: 0.1),
                    borderRadius: BorderRadius.circular(L.responsiveSize(phone: 8, tablet: 10, desktop: 12)),
                    border: Border.all(
  ...
  static Color _defaultColor(TransactionType type, BuildContext context) {
    ...
    return switch (type) {
      TransactionType.income => theme.colorScheme.secondary,
      TransactionType.expense => theme.colorScheme.error,
      TransactionType.transfer => theme.colorScheme.tertiary,
    };
  }
